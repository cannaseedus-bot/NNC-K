...
| `src/NeuralGrammar.Core/GPUComputePipeline.cs` | `NeuralGrammar.Core.XCFE` | GPUComputePipeline, KernelType, GpuKernel, ComputeResult | GPU Compute Pipeline — generates compute kernels for D3D11_1 (HLSL cs_5_0), WebGL2 (GLSL ES 300), and OpenCL C 1.2, and dispatches through D3D11_1/WebGL2; OpenCL C is generated but not natively dispatched (reach lane — see the XVM backend matrix). Falls back to CPU when no GPU back... |
